function gridSearch(G: string[], P: string[]) {
  //step 1
  ...
  const trigger: string = P[0][0]; //the first character of the first string
  const possibilityRow: number = G.length - P.length; //the possible rows where our search string array can occur
  const possibilityColumn: number = G[0].length - P[0].length; //the possible columns where our search string array can occur
...
function finder(
  ...
  while (i <= _possibilityColumn) {
    if (
      _rowString[i] === _trigger &&
      _rowString.substring(i, P[0].length + i) === P[0]
    ) {
...
function checker(__s: number, __i: number) {
