...
function getTable (s) {
    ...
    let rows = (Number.isInteger(root)) ? root : Math.trunc(root)
    let columns = (Number.isInteger(root)) ? root : Math.trunc(root)+ 1
...
function getStrBlock(rows, columns, fIndex, s){
    ...
    while (i < rows){
        ...
        let index = indexCalculator(i, columns, fIndex) + fIndex
...
function indexCalculator(i, columns, fIndex) {
    return i * columns
}
